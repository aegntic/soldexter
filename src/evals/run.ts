/**
 * LangSmith Evaluation Runner for Soldexter
 * 
 * Usage:
...
// ============================================================================
// Target function - wraps Soldexter agent
// ============================================================================
...
function createEvaluationRunner(sampleSize?: number) {
  return async function* runEvaluation(): AsyncGenerator<EvalProgressEvent, void, unknown> {
    ...
    const datasetName = sampleSize 
      ? `soldexter-eval-sample-${sampleSize}-${Date.now()}`
      : 'soldexter-eval';
    ...
    const experimentName = `soldexter-eval-${Date.now().toString(36)}`;
